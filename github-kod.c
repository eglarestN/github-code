...
void packet_handler(unsigned char *user_data, const struct pcap_pkthdr *pkthdr, const unsigned char *packet) {
    ...
    fprintf(traffic_file, "Source IP: %d.%d.%d.%d\n", packet[26], packet[27], packet[28], packet[29]);
    fprintf(traffic_file, "Destination IP: %d.%d.%d.%d\n", packet[30], packet[31], packet[32], packet[33]);
    fprintf(traffic_file, "Source MAC: %02x:%02x:%02x:%02x:%02x:%02x\n", packet[6], packet[7], packet[8], packet[9], packet[10], packet[11]);
    fprintf(traffic_file, "Source Port: %u\n", (packet[36] << 8) | packet[37]);
    fprintf(traffic_file, "Destination Port: %u\n", (packet[38] << 8) | packet[39]);
